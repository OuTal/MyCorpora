...
if(!isset($_SESSION['next']) || isset($_GET['n'])){
	if(isset($_GET['n'])){
...
<div style="align:center;width:100%;background:#000000;padding:10px;">
  ...
  <div class="limiter">
    ...
    <h3 style="margin-top: 5%">Phrase N°<br>
    <?php 
    if(isset($_GET['next']) && !isset($_GET['n']))
    {
       $cle = (int)$_GET['next'];
    ...
    ?>
        / <?php echo count($prompts).' '.$cle.' '.$subc.' '.$_SESSION["cat"].' ';?>
    </h3>
...
<a href="<?php 
		$idd=$_SESSION['id'];
		if(isset($_GET['n'])){
			$lienrec="Demo/recordings/$idd/audio_".$_GET['nnn'].".wav";
		...
		if ($cle!=1){
			$lienrec="Demo/recordings/$idd/audio_".$prompts[$cle-2]['phrase'].".wav";
			$_SESSION['sub']++;
       ...
       if($cle < count($prompts)){echo 'nv_index.php?next='.$cle; }
  ...
  <div class = "liste" style="margin-bottom: 2%">
  ...
  <ul id="recordingslist">Votre enregistrement s'affichera ici !</ul>
  					...
  					<div style="margin-bottom: 2%">
						<span>
						<a href="<?php
							if($cle < count($prompts)){echo 'nv_index.php?n='.$_SESSION['form'].'&nn='.$prompts[$cle-1]['Id'].'&nnn='.$prompts[$cle-1]['phrase']; }
							else {echo '../vue/Thanks.php?n='.$_SESSION['form'].'&nn='.$prompts[$cle-1]['Id'].'&nnn='.$prompts[$cle-1]['phrase'];}
						?>" style="color:#2E64FE">
						Changer de catégorie
						</a> - <a href="<?php echo '../vue/Thanks.php?n=1';	?>" style="color:red">
...
var fileName = 'audio_<?php echo $phrase;?>.wav';  // or "wav"
